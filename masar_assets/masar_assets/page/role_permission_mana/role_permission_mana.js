frappe.pages['role-permission-mana'].on_page_load = function(wrapper) {
	...
	$(frappe.render_template("custom_permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new frappe.PermissionEngine(wrapper);
};

frappe.pages["role-permission-mana"].refresh = function (wrapper) {
...
frappe.PermissionEngine = class PermissionEngine {
	...
	make() {
		...
		frappe
			.call({
				method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.get_roles_and_doctypes",
			})
			.then((res) => {
	...
	setup_page() {
		...
		console.log(this.doctype_select);
	...
	get_standard_permissions(callback) {
		...
		if (doctype) {
			return frappe.call({
				method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.get_standard_permissions",
	...
	reset_std_permissions(data) {
		...
		let d = frappe.confirm(__("Reset Permissions for {0}?", [doctype]), () => {
			return frappe
				.call({
					method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.reset",
					args: { doctype },
				})
				.then(() => {
					this.refresh();
	...
	refresh() {
		...
		frappe
			.call({
				method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.get_permissions",
				args: { doctype, role },
			})
			.then((r) => {
	...
	set_show_users(cell, role) {
		cell.html("<a class='grey' href='#'>" + __(role) + "</a>")
			...
			.click(function () {
				...
				frappe.call({
					method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.get_users_with_role",
					args: {
						role: role,
					},
					callback: function (r) {
	...
	add_delete_button(row, d) {
		$(
			...
			.on("click", () => {
				return frappe.call({
					method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.remove",
					args: {
						doctype: d.parent,
						role: d.role,
	...
	add_check_events() {
		...
		this.body.on("click", "input[type='checkbox']", function () {
			...
			return frappe.call({
				method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.update",
				args: args,
				callback: (r) => {
					frappe.dom.unfreeze();
					if (r.exc) {
		...
		this.page.set_primary_action(
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					frappe.call({
						method: "masar_assets.masar_assets.page.role_permission_mana._permission_manager.add",
						args: args,
						callback: (r) => {
							if (r.exc) {
